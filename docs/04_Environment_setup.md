...
- **Base image `espressif/idf:release-v5.3`** — pulls the exact toolchain (GCC, Python, CMake, `idf.py`) that Espressif ships. Zero local installs.
- **`--privileged` + `--device=/dev/ttyACM0`** — required for USB-JTAG passthrough. Without `--privileged`, OpenOCD cannot claim the USB endpoint.
- **`postCreateCommand` sources `export.sh`** — every new terminal has `idf.py` on `PATH` without manual sourcing.
...
`sdkconfig` is generated and git-ignored. `sdkconfig.defaults` is the source of truth, and it is intentionally small (six lines). The important ones:
...
# Add this to host ~/.bashrc so the agent always runs:
